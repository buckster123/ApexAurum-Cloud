...
Combines:
- ACT-R base-level activation: B(t) = ln(Sigma t_k^{-d})
...
from app.cerebro.models.memory import StrengthState
...
def base_level_activation(
    ...
    """Compute ACT-R base-level activation B(t) = ln(Sigma t_k^{-d}).
...
def combined_recall_score(
    ...
    Blends four signals:
    - Vector similarity (semantic relevance from pgvector)
