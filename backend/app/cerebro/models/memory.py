...
from app.cerebro.types import EmotionalValence, MemoryLayer, MemoryType, Visibility
...
class StrengthState(BaseModel):
    ...
    # Access history for ACT-R base-level activation B(t) = ln(Sigma t_k^{-d})
...
class MemoryNode(BaseModel):
    ...
    Stored in PostgreSQL with pgvector for embeddings and
    cerebro_associative_links for graph topology.
    """
    ...
    # Cached graph info (populated from graph store, not stored in embedding)
